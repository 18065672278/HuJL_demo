...
Component({
  ...
  ready: function() {
    // 如果初始化有值，赋值，否则，置为空
    this.setData({
      carNum: this.data.initValue != '请填写车牌号/车架号' && this.data.initValue != '请填写车牌号' ? this.data.initValue : ''
    })
  },
  ...
  methods: {
    ...
    _handleResult: function() {
      const myEventDetail = {
        value: this.data.carNum // 传递到结果文本
      };
      // 触发外部绑定事件，传递结果参数
      this.triggerEvent('getResult', myEventDetail);
    },
    /**
     * 键盘主要键点击事件，将点击内容更新到carNum
     */
    ...
    _handleDelete: function() {
      let currentText = this.data.carNum;
