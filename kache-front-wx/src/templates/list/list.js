const app = getApp();

Page({

  /**
   * 页面的初始数据
   */
  data: {
    product_list: [{
      color: "玫瑰金",
      cover: "https://i8.mifile.cn/v1/a1/7dc38112-bbf4-f3fa-db39-5f4674f9d0d4!720x792.webp",
      goods_name: "红米5",
      isTouchMove: false,
      memory: "",
      price: "1000",
      select_num: 1,
      remark: 'asdasdafdsfsfsdfdsg',
      selected: false,
      unSelect: true
    }, {
      color: "黑色",
      cover: "https://i8.mifile.cn/v1/a1/7dc38112-bbf4-f3fa-db39-5f4674f9d0d4!720x792.webp",
      goods_name: "红米5",
      isTouchMove: false,
      memory: "",
      price: "1000",
      select_num: 1,
      selected: false,
    }, {
      color: "玫瑰金",
      cover: "https://i8.mifile.cn/v1/a1/7dc38112-bbf4-f3fa-db39-5f4674f9d0d4!720x792.webp",
      goods_name: "红米5",
      isTouchMove: false,
      memory: "",
      price: "",
      select_num: 1,
      selected: false,
    }, {
      color: "黑色",
      cover: "https://i8.mifile.cn/v1/a1/7dc38112-bbf4-f3fa-db39-5f4674f9d0d4!720x792.webp",
      goods_name: "红米5",
      isTouchMove: false,
      memory: "",
      price: "",
      select_num: 1,
      selected: false,
    }, {
      color: "玫瑰金",
      cover: "https://i8.mifile.cn/v1/a1/7dc38112-bbf4-f3fa-db39-5f4674f9d0d4!720x792.webp",
      goods_name: "红米5",
      isTouchMove: false,
      memory: "",
      price: "",
      select_num: 1,
      selected: false,
    }, {
      color: "黑色",
      cover: "https://i8.mifile.cn/v1/a1/7dc38112-bbf4-f3fa-db39-5f4674f9d0d4!720x792.webp",
      goods_name: "红米5",
      isTouchMove: false,
      memory: "",
      price: "",
      select_num: 1,
      selected: false,
    }, {
      color: "玫瑰金",
      cover: "https://i8.mifile.cn/v1/a1/7dc38112-bbf4-f3fa-db39-5f4674f9d0d4!720x792.webp",
      goods_name: "红米5",
      isTouchMove: false,
      memory: "",
      price: "",
      select_num: 1,
      selected: false,
    }, {
      color: "黑色",
      cover: "https://i8.mifile.cn/v1/a1/7dc38112-bbf4-f3fa-db39-5f4674f9d0d4!720x792.webp",
      goods_name: "红米5",
      isTouchMove: false,
      memory: "",
      price: "",
      select_num: 1,
      selected: false,
    }, {
      color: "玫瑰金",
      cover: "https://i8.mifile.cn/v1/a1/7dc38112-bbf4-f3fa-db39-5f4674f9d0d4!720x792.webp",
      goods_name: "红米5",
      isTouchMove: false,
      memory: "",
      price: "",
      select_num: 1,
      selected: false,
    }, {
      color: "黑色",
      cover: "https://i8.mifile.cn/v1/a1/7dc38112-bbf4-f3fa-db39-5f4674f9d0d4!720x792.webp",
      goods_name: "红米5",
      isTouchMove: false,
      memory: "",
      price: "",
      select_num: 1,
      selected: false,
    }],
    totalPrice: 0,
    selectAllStatus: false,
    startX: 0, //开始坐标
    startY: 0
  },
  //选中
  selectList(e) {
    let selectAllStatus = this.data.selectAllStatus;
    const index = e.currentTarget.dataset.index;
    let product_list = this.data.product_list;
    const selected = product_list[index].selected;
    product_list[index].selected = !selected;
    console.log(selected);
    //购物车列表里的条目只要有一个取消，全选就取消
    const symbol = product_list.some(product => {
      return product.selected === false;
    });
    if (symbol) {
      this.data.selectAllStatus = false;
    } else {
      this.data.selectAllStatus = true;
    }

    this.setData({
      product_list,
      selectAllStatus: this.data.selectAllStatus
    });
  },
  //左滑开始
  touchstart: function (e) {
    this.data.product_list.map(item => {
      if (item.isTouchMove) {
        item.isTouchMove = false;
      }
    })
    this.setData({
      startX: e.changedTouches[0].clientX,
      startY: e.changedTouches[0].clientY,
      product_list: this.data.product_list
    })
  },
  //滑动事件处理
  touchmove(e) {
    var
      index = e.currentTarget.dataset.index, //当前索引
      startX = this.data.startX, //开始X坐标
      startY = this.data.startY, //开始Y坐标
      touchMoveX = e.changedTouches[0].clientX, //滑动变化坐标
      touchMoveY = e.changedTouches[0].clientY, //滑动变化坐标
      //获取滑动角度
      angle = this.angle({
        X: startX,
        Y: startY
      }, {
          X: touchMoveX,
          Y: touchMoveY
        });
    this.data.product_list.forEach(function (v, i) {
      v.isTouchMove = false
      //滑动超过30度角 return
      if (Math.abs(angle) > 30) return;
      if (i == index) {
        if (touchMoveX > startX) //右滑
          v.isTouchMove = false
        else //左滑
          v.isTouchMove = true
      }
    })
    //更新数据
    this.setData({
      product_list: this.data.product_list
    })
  },
  angle(start, end) {
    var X = end.X - start.X,
      Y = end.Y - start.Y
    //返回角度 /Math.atan()返回数字的反正切值
    return 360 * Math.atan(Y / X) / (2 * Math.PI);
  },
  //删除
  delproductItem(e) {
    const index = e.currentTarget.dataset.index;
    console.log(index);
    this.data.product_list.splice(index, 1);
    wx.clearStorageSync("select_num");
    this.setData({
      product_list: this.data.product_list
    });
  },
  //查看详情
  showProductDetail(e) {
    console.log(e.currentTarget.dataset.index);
  },
  /**
   * 生命周期函数--监听页面加载
   */
  onLoad: function (options) {
    this.setData({
      hasSelect: true
    })
  },

  /**
   * 生命周期函数--监听页面初次渲染完成
   */
  onReady: function () {

  },

  /**
   * 生命周期函数--监听页面显示
   */
  onShow: function () {
    const attr_item = this.data.product_list;
    console.log(attr_item)
    // return
    let product_list = this.data.product_list;
    // arr = [attr_item, ...arr]
    product_list = [...attr_item];
    // arr.push(temp);
    console.log(product_list);
    const select_num = product_list.map(item => {
      return item.select_num;
    })
    // console.log(select_num);
    let goods_sum = 0;
    for (let i = 0, len = select_num.length; i < len; i++) {
      goods_sum += select_num[i];
    }
    wx.setStorageSync('goods_sum', goods_sum);
    console.log(goods_sum);
    // console.log(temp)
    this.setData({
      product_list
    });
    console.log(this.data.product_list);
  },

  /**
   * 生命周期函数--监听页面隐藏
   */
  onHide: function () {

  },

  /**
   * 生命周期函数--监听页面卸载
   */
  onUnload: function () {

  },

  /**
   * 页面相关事件处理函数--监听用户下拉动作
   */
  onPullDownRefresh: function () {

  },

  /**
   * 页面上拉触底事件的处理函数
   */
  onReachBottom: function () {

  },

  /**
   * 用户点击右上角分享
   */
  onShareAppMessage: function () {

  }
})