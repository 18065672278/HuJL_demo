...
Page({
  ...
  showProductDetail: function(e) {
    console.log(e);
    wx.navigateTo({
      url: '../legalPersonEdit/index?id=' + e.currentTarget.dataset.index,
    })
  },
  //选中
  selectList(e) {
  ...
  delProductItem(e) {
    ...
    WXEXT.showModal('', "是否确定删除", true, '确定删除', '取消', function(data) {
      if (data.confirm) {
        ...
        WXEXT.request(API_SERVER + API_api.firmdelete, {
          ids: id
        }, '', me.delcallback)
      }
    });
  ...
  query: function(queryNext, searchNext) {
    ...
    if (searchNext && searchinput.length != 0) {
      s.legalName = searchinput
    }
    WXEXT.request(API_SERVER + API_api.firmquery, s, '', me.callback)
  },
  ...
  productList: function(favorList) {
    ...
    for (let i = 0, length = favorList.length; i < length; i++) {
      ...
      listA.secondRowR = cui.isEmpty(favorList[i].legalTel) ? "" : favorList[i].legalTel;
      listA.remark = cui.isEmpty(favorList[i].creditCode) ? "" : favorList[i].creditCode;
