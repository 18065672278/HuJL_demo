...
Page({
  ...
  showProductDetail: function(e) {
    ...
    console.log()
    wx.navigateTo({
      url: '../driverEdit/index?id=' + e.currentTarget.dataset.index,
    })
  },
  //选中
  selectList(e) {
  ...
  checkOut: function(e) {
    ...
    product_list.forEach(function(v, i) {
      if (v.selected == true) {
        ...
        WXEXT.request(API_SERVER + API_api.driversaveBandInfo, s, '', function(data) {
          wx.showToast({
            title: '绑定成功',
          }, wx.navigateBack({}))
        })
  ...
  delProductItem(e) {
    ...
    WXEXT.showModal('', "是否确定删除", true, '确定删除', '取消', function(data) {
      if (data.confirm) {
        ...
        WXEXT.request(API_SERVER + API_api.driverdelete, {
          ids: id
        }, '', me.delcallback)
      }
    });
  ...
  query: function(queryNext, searchNext) {
    ...
    WXEXT.request(API_SERVER + API_api.driverquery, s, '', me.callback)
  },
  //查询回调
  ...
  productList: function(favorList) {
    ...
    for (let i = 0, length = favorList.length; i < length; i++) {
      ...
      listA.firstRowR = cui.isEmpty(favorList[i].mobile) ? "" : favorList[i].mobile;
      listA.secondRowL = cui.isEmpty(favorList[i].certNum) ? "" : favorList[i].certNum;
      listA.secondRowR = cui.isEmpty(favorList[i].nextResetDate) ? "" : favorList[i].nextResetDate;
