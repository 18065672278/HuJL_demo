...
Page({
  ...
  showProductDetail: function(e) {
    console.log(e);
    wx.navigateTo({
      url: '../billDetail/index?id=' + e.currentTarget.dataset.index,
    })
  },
  //左滑开始
  ...
  productList: function(favorList) {
    ...
    for (let i = 0, length = favorList.length; i < length; i++) {
      ...
      listA.firstRowL = cui.isEmpty(favorList[i].shopname) ? "" : favorList[i].shopname;
      listA.firstRowR = cui.isEmpty(favorList[i].shopcost) ? "" : favorList[i].shopcost;
      listA.secondRowL = cui.isEmpty(favorList[i].shoptype) ? "" : favorList[i].shoptype;
      listA.secondRowR = cui.isEmpty(favorList[i].shopstatus) ? "" : favorList[i].shopstatus;
      listA.remark = cui.isEmpty(favorList[i].shopdate) ? "" : favorList[i].shopdate;
