...
Page({
  ...
  addBtn: function(e) {
    wx.navigateTo({
      url: '../../personBox/person/index?select=true&&type=outlet&&outletId=' + app.globalData.userInfo.outletList[0].id,
    })
  },
  //编辑
  showProductDetail: function(e) {
    wx.navigateTo({
      url: '../../personnelDetail/index?type=outlet&&id=' + e.currentTarget.dataset.index,
    })
  },
  //左滑开始
  ...
  delProductItem(e) {
    ...
    WXEXT.showModal('', "是否确定删除", true, '确定删除', '取消', function(data) {
      if (data.confirm) {
        ...
        WXEXT.request(API_SERVER + API_api.outletEmployeeUnBand, {
          id: id,
  ...
  query: function(queryNext, searchNext) {
    ...
    WXEXT.request(API_SERVER + API_api.outletEmployeequery, s, '', me.callback)
  },
  //查询回调
