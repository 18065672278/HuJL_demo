...
Page({
  ...
  showProductDetail: function(e) {
    console.log(e);
    wx.navigateTo({
      url: '../outletEdit/index?outletCode=' + e.currentTarget.dataset.index,
    })
  },
  //选中
  selectList(e) {
  ...
  checkOut: function(e) {
    ...
    product_list.forEach(function(v, i) {
      if (v.selected == true) {
        ...
        WXEXT.request(API_SERVER + API_api.fleetOutletsaveBandInfo, s, '', me.selectback)
        wx.navigateBack({})
      }
    });
  },
  ...
  delProductItem(e) {
    ...
    WXEXT.showModal('', "是否确定删除", true, '确定删除', '取消', function(data) {
      if (data.confirm) {
        ...
        WXEXT.request(API_SERVER + API_api.outletdelete, {
          outletCodes: id
        }, '', me.delcallback)
      }
    });
      ...
      success: function(res) {
        ...
        qqmapsdk.reverseGeocoder({
          ...
          success: function(res) {
            //获取当前地址成功
            console.log(res);
            orgCode = res.result.ad_info.adcode;
            if (orgCode != "" && latitude != "" && longitude != "") {
              var s = {
                orgCode: orgCode,
                latitude: latitude,
                longitude: longitude,
  ...
  productList: function(favorList) {
    ...
    for (let i = 0, length = favorList.length; i < length; i++) {
      let listA = {};
      listA.id = favorList[i].outletCode;
      ...
      listA.outletId = cui.isEmpty(favorList[i].id) ? "" : favorList[i].id;
