...
Page({
  ...
  addBtn: function(e) {
    wx.navigateTo({
      url: '../serviceItemEdit/index?outletCode=' + this.data.outletCode,
    })
  },
  //编辑
  showProductDetail: function(e) {
    console.log(e);
    wx.navigateTo({
      url: '../serviceItemEdit/index?id=' + e.currentTarget.dataset.index + '&outletCode=' + this.data.outletCode,
    })
  },
  //左滑开始
  ...
  delProductItem(e) {
    ...
    WXEXT.showModal('', "是否确定删除", true, '确定删除', '取消', function(data) {
      if (data.confirm) {
        ...
        WXEXT.request(API_SERVER + API_api.outletServiceItemdelete, {
          ids: id
        }, '', me.delcallback)
      }
    });
  ...
  query: function(queryNext, searchNext) {
    ...
    if (searchNext && searchinput.length != 0) {
      s.serviceName = searchinput
    }
    WXEXT.request(API_SERVER + API_api.outletServiceItemquery, s, '', me.callback)
  },
  //查询回调
  ...
  productList: function(favorList) {
    ...
    for (let i = 0, length = favorList.length; i < length; i++) {
      ...
      listA.firstRowL = cui.isEmpty(favorList[i].serviceName) ? "" : '服务项名称：' + favorList[i].serviceName;
      listA.firstRowR = "";
      listA.secondRowL = cui.isEmpty(favorList[i].servicePrice) ? "" : '价格：' + favorList[i].servicePrice + '￥';
      listA.secondRowR = cui.isEmpty(favorList[i].dayTime) ? "" : favorList[i].dayTime == 1 ? '服务时间：白天' : '服务时间：晚上';
