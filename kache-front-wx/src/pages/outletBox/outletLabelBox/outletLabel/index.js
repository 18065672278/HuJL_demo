...
Page({
  ...
  addBtn: function(e) {
    wx.navigateTo({
      url: '../outletLabelEdit/index?outletCode=' + this.data.outletCode,
    })
  },
  //编辑
  showProductDetail: function(e) {
    console.log(e);
    wx.navigateTo({
      url: '../outletLabelEdit/index?id=' + e.currentTarget.dataset.index + '&outletCode=' + this.data.outletCode,
    })
  },
  //左滑开始
  ...
  delProductItem(e) {
    ...
    WXEXT.showModal('', "是否确定删除", true, '确定删除', '取消', function(data) {
      if (data.confirm) {
        ...
        WXEXT.request(API_SERVER + API_api.outletLabeldelete, {
          ids: id
        }, '', me.delcallback)
      }
    });
  ...
  query: function(queryNext, searchNext) {
    ...
    if (searchNext && searchinput.length != 0) {
      s.labelName = searchinput
    }
    WXEXT.request(API_SERVER + API_api.outletLabelquery, s, '', me.callback)
  },
  //查询回调
  ...
  productList: function(favorList) {
    ...
    for (let i = 0, length = favorList.length; i < length; i++) {
      ...
      listA.firstRowL = cui.isEmpty(favorList[i].labelName) ? "" : '服务标签名称：' + favorList[i].labelName;
      listA.firstRowR = "";
      listA.secondRowL = cui.isEmpty(favorList[i].labelValue) ? "" : '标签值：' + favorList[i].labelValue;
      listA.secondRowR = cui.isEmpty(favorList[i].orderNo) ? "" : '排序：' + favorList[i].orderNo;
