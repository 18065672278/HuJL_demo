...
Page({
  ...
  addBtn: function(e) {
    wx.navigateTo({
      url: '../businessLicenseInfoEdit/index',
    })
  },
  //编辑
  showProductDetail: function(e) {
    console.log(e);
    wx.navigateTo({
      url: '../businessLicenseInfoEdit/index?id=' + e.currentTarget.dataset.index,
    })
  },
  //选中
  selectList(e) {
  ...
  delProductItem(e) {
    ...
    WXEXT.showModal('', "是否确定删除", true, '确定删除', '取消', function(data) {
      if (data.confirm) {
        ...
        WXEXT.request(API_SERVER + API_api.certdelete, {
          ids: id
        }, '', me.delcallback)
      }
    });
  ...
  query: function(queryNext, searchNext) {
    ...
    if (searchNext && searchinput.length != 0) {
      s['identityCert.name'] = searchinput;
    }
    WXEXT.request(API_SERVER + API_api.certquery, s, '', me.callback)
  },
  ...
  productList: function(favorList) {
    ...
    for (let i = 0, length = favorList.length; i < length; i++) {
      let listA = {};
      listA.id = favorList[i].certId;
      listA.img = "https://i8.mifile.cn/v1/a1/7dc38112-bbf4-f3fa-db39-5f4674f9d0d4!720x792.webp";
      listA.firstRowL = cui.isEmpty(favorList[i].companyName) ? "" : favorList[i].companyName;
      listA.firstRowR = "";
      listA.secondRowL = cui.isEmpty(favorList[i].companyAddress) ? "" : favorList[i].companyAddress;
      listA.secondRowR = cui.isEmpty(favorList[i].creditCode) ? "" : favorList[i].creditCode;
