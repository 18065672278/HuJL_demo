...
Page({
  ...
  addBtn: function(e) {
    wx.navigateTo({
      url: '../openingPermitInfoEdit/index',
    })
  },
  //编辑
  showProductDetail: function(e) {
    console.log(e);
    wx.navigateTo({
      url: '../openingPermitInfoEdit/index?id=' + e.currentTarget.dataset.index,
    })
  },
  //选中
  selectList(e) {
  ...
  query: function(queryNext, searchNext) {
    ...
    if (searchNext && searchinput.length != 0) {
      s['openingCert.name'] = searchinput;
    }
  ...
  productList: function(favorList) {
    ...
    for (let i = 0, length = favorList.length; i < length; i++) {
      ...
      listA.firstRowR = "";
      listA.secondRowL = cui.isEmpty(favorList[i].legalName) ? "" : favorList[i].legalName;
