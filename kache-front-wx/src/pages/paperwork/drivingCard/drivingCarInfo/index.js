...
Page({
  ...
  showProductDetail: function(e) {
    console.log(e);
    wx.navigateTo({
      url: '../drivingCarInfoEdit/index?id=' + e.currentTarget.dataset.index,
    })
  },
  //选中
  selectList(e) {
  ...
  query: function(queryNext, searchNext) {
    ...
    if (searchNext && searchinput.length != 0) {
      s['vehicleCert.name'] = searchinput;
    }
  ...
  productList: function(favorList) {
    ...
    for (let i = 0, length = favorList.length; i < length; i++) {
      ...
      listA.firstRowR = "";
      listA.secondRowL = cui.isEmpty(favorList[i].ownerName) ? "" : favorList[i].ownerName;
      listA.secondRowR = cui.isEmpty(favorList[i].vinNum) ? "" : favorList[i].vinNum;
