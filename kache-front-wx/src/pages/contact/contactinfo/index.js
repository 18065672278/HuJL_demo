...
Page({
  ...
  addBtn: function(e) {
    wx.navigateTo({
      url: '../contactinfoEdit/index?personId=' + this.data.personId,
    })
  },
  //编辑
  showProductDetail: function(e) {
    console.log(e);
    wx.navigateTo({
      url: '../contactinfoEdit/index?id=' + e.currentTarget.dataset.index + '&personId=' + this.data.personId,
    })
  },
  //选中
  selectList(e) {
  ...
  delProductItem(e) {
    ...
    WXEXT.showModal('', "是否确定删除", true, '确定删除', '取消', function(data) {
      if (data.confirm) {
        ...
        WXEXT.request(API_SERVER + API_api.contacdelete, {
          ids: id
        }, '', me.delcallback)
      }
    });
  ...
  query: function(queryNext, searchNext) {
    ...
    if (searchNext && searchinput.length != 0) {
      s.fleetName = searchinput
    }
    WXEXT.request(API_SERVER + API_api.contactquery, s, '', me.callback)
  },
  //查询回调
  ...
  productList: function(favorList) {
    ...
    for (let i = 0, length = favorList.length; i < length; i++) {
      ...
      listA.firstRowL = cui.isEmpty(favorList[i].address) ? "" : favorList[i].address.replace(/(\[address]|,)/gi, ' ');
      listA.firstRowR = "";
      listA.secondRowL = cui.isEmpty(favorList[i].addressAlias) ? "" : favorList[i].addressAlias;
      listA.secondRowR = cui.isEmpty(favorList[i].num1) ? "" : favorList[i].num1;
  ...
  onLoad: function(options) {
    console.log(options.personId != '', options.personId)
