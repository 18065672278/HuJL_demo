...
Page({
  ...
  showProductDetail: function(e) {
    ...
    console.log()
    wx.navigateTo({
      url: '../personEdit/index?id=' + e.currentTarget.dataset.index,
    })
  },
  //选中
  selectList(e) {
  ...
  checkOut: function(e) {
    let me = this,
      product_list = me.data.product.product_list;
    product_list.forEach(function(v, i) {
      if (v.selected == true) {
        console.log(v);
        if (!cui.isEmpty(me.data.hasSelectType) && me.data.hasSelectType == 'outlet') {
          console.log(v.id);
          WXEXT.request(API_SERVER + API_api.outletEmployeeBandInfo, {
            personId: v.id,
            outletId: me.data.outletId
          }, '', me.checkOutback)
        } else if (!cui.isEmpty(me.data.hasSelectType) && me.data.hasSelectType == 'fleet') {
          console.log(v.id);
          WXEXT.request(API_SERVER + API_api.fleetEmployeeBandInfo, {
            personId: v.id,
  ...
  delProductItem(e) {
    ...
    WXEXT.showModal('', "是否确定删除", true, '确定删除', '取消', function(data) {
      if (data.confirm) {
        ...
        WXEXT.request(API_SERVER + API_api.persondelete, {
          ids: id
        }, '', me.delcallback)
      }
    });
  ...
  query: function(queryNext, searchNext) {
    ...
    WXEXT.request(API_SERVER + API_api.personquery, s, '', me.callback)
  },
  //查询回调
  ...
  onLoad: function(options) {
    ...
    if (!cui.isEmpty(options.outletId)) {
      this.setData({
        hasSelectType: options.type,
        outletId: options.outletId
      })
    }
    if (!cui.isEmpty(options.fleetId)) {
      this.setData({
        hasSelectType: options.type,
