...
Page({
  ...
  showProductDetail: function (e) {
    ...
    console.log()
    wx.navigateTo({
      url: '../fleetEdit/index?id=' + e.currentTarget.dataset.index,
    })
  },
  //左滑开始
  ...
  query: function (queryNext, searchNext) {
    ...
    WXEXT.request(API_SERVER + API_api.fleetquery, s, '', me.callback)
  },
  //查询回调
  ...
  productList: function (favorList) {
    ...
    for (let i = 0, length = favorList.length; i < length; i++) {
      ...
      listA.firstRowL = cui.isEmpty(favorList[i].fleetName) ? "" : favorList[i].fleetName;
      listA.firstRowR = "";
      listA.secondRowL = cui.isEmpty(favorList[i].orgCode) ? "" : favorList[i].orgCode;
      listA.secondRowR = cui.isEmpty(favorList[i].principalTel) ? "" : favorList[i].principalTel;
