...
Page({
  ...
  addBtn: function(e) {
    wx.navigateTo({
      url: '../../outletBox/outletList/index?select=true&&fleetId=' + this.data.fleetId,
    })
  },
  //编辑
  showProductDetail: function(e) {
    ...
    this.data.product.product_list.forEach(function(v, i) {
      ...
      if (v.id == outletId) {
        ...
        wx.navigateTo({
          url: '../../outletBox/outletEdit/index?outletCode=' + outletCode,
        })
      }
    })
  },
  ...
  delProductItem(e) {
    ...
    WXEXT.showModal('', "是否确定删除", true, '确定删除', '取消', function(data) {
      if (data.confirm) {
        ...
        WXEXT.request(API_SERVER + API_api.fleetOutletsaveUnBandInfo, {
  ...
  query: function(queryNext, searchNext) {
    ...
    WXEXT.request(API_SERVER + API_api.fleetOutletqueryBindOutlet, s, '', me.callback)
  },
  //查询回调
  ...
  productList: function(favorList) {
    ...
    for (let i = 0, length = favorList.length; i < length; i++) {
      ...
      listA.outletCode = favorList[i].outletCode;
