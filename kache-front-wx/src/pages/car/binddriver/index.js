...
Page({
  ...
  addBtn: function(e) {
    wx.navigateTo({
      url: '../../driverBox/driver/index?select=true&&carId=' + this.data.carId,
    })
  },
  //编辑
  showProductDetail: function(e) {
    console.log(e);
    wx.navigateTo({
      url: '../../driverBox/driverEdit/index?id=' + e.currentTarget.dataset.index,
    })
  },
  //左滑开始
  ...
  delProductItem(e) {
    ...
    WXEXT.showModal('', "是否确定删除", true, '确定删除', '取消', function(data) {
      if (data.confirm) {
        ...
        WXEXT.request(API_SERVER + API_api.driversaveUnBandInfo, {
          carId: me.data.carId,
          personId: id
        }, '', me.delcallback)
      }
  ...
  query: function(queryNext, searchNext) {
    ...
    WXEXT.request(API_SERVER + API_api.driverbandQuery, s, '', me.callback)
  },
  //查询回调
