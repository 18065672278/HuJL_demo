 ...
 */
 function selectOrgTreeadress(para,callBackName){
    ...
    if(callBackName){
        var html = '<input id="pop-address-options" type="hidden" data-callBack="'+callBackName+'" data-orgName="'+parameters.orgName+'" data-orgId="'+parameters.orgId+'" data-type="'+parameters.type+'" data-orgType="'+parameters.orgType+'" data-isMulti="'+parameters.isMulti+'"/>'; 
    }else{
        var html = '<input id="pop-address-options" type="hidden" data-orgName="'+parameters.orgName+'" data-orgId="'+parameters.orgId+'"  data-type="'+parameters.type+'" data-orgType="'+parameters.orgType+'" data-isMulti="'+parameters.isMulti+'"/>'; 
    }
    ...
    if(parameters.isTop){
        top.layer.open({
            ...
            shadeClose: false,
            content:["setting/address_tree.html",'no']
        });
    }else{
        layer.open({
            type: 2,
